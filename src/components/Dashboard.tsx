import { DollarSign, Building2, TrendingUp, Calendar, Users, ArrowUpRight, MoreHorizontal, Plus, ExternalLink, RefreshCw, Home, Wifi, LayoutDashboard, Settings } from 'lucide-react';
import { useEffect, useState } from 'react';
import type React from 'react';
import type { Property } from '../types';
import { getAllProperties } from '../services/propertyService';
import HousekeepingOps from './HousekeepingOps';
    ...
    <div className="space-y-5">
      ...
      <div className="flex items-center gap-1 bg-slate-100 p-1 rounded-xl w-fit">
        ...
        <button
          ...
          className={`px-4 py-2 rounded-lg text-sm font-semibold transition-colors flex items-center gap-1.5 ${
            activeTab === 'operations' ? 'bg-white text-slate-900 shadow-sm' : 'text-slate-500 hover:text-slate-700'
          }`}
        >
          <Settings size={15} /> Operations
        </button>
      </div>
      ...
      {activeTab === 'overview' ? (
        <div className="space-y-6">
          <div className="grid grid-cols-1 sm:grid-cols-2 lg:grid-cols-4 gap-5">
            <KpiCard label="Estimated Revenue" value="$0" sub="+0%" icon={<DollarSign size={18} />} trend="neutral" />
            <KpiCard label="Active Properties" value={String(activeCount)} sub="Real-time" icon={<Building2 size={18} />} trend="up" />
            <KpiCard label="Avg Nightly Rate" value={`$${avgRate}`} sub={`${activeCount} units`} icon={<TrendingUp size={18} />} trend="up" />
            <KpiCard label="Direct Bookings" value="0" sub="No bookings yet" icon={<Calendar size={18} />} trend="neutral" />
          </div>
                    ...
                    {CHART_DATA.map((val, i) => {
                      ...
                      return (
                        <g key={i} className="group">
                          <rect x={x} y={y} width={barWidth} height={barHeight} rx="4" fill="#4f46e5" className="transition-all duration-300" style={{ transition: 'fill 0.3s' }} />
                        ...
                        {properties.slice(0, 6).map(p => (
                          <tr key={p.id} className="hover:bg-slate-50/60 transition-colors group">
                            ...
                            <td className="py-2.5 px-4">
                              <span className="inline-flex items-center px-2 py-0.5 rounded-full text-[11px] font-semibold bg-emerald-50 text-emerald-700 ring-1 ring-emerald-100">{p.status}</span>
                            </td>
                            <td className="py-2.5 px-6">
        ...
        <HousekeepingOps properties={properties} />
      )}
    </div>
  );
}
